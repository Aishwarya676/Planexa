document.addEventListener('DOMContentLoaded', () => {
    ...
    try {
      ...
      if (coachImgEl) {
        if (currentCoach.profile_photo) {
          ...
          coachImgEl.innerText = currentCoach.name.charAt(0).toUpperCase();
        }
      }
  ...
  async function loadChatMessages() {
    ...
    try {
      const msgs = await api.get(`/api/messages/${currentCoach.id}`);
      renderMessages(msgs);
      scrollToBottom();
    } catch (e) {
  ...
  function renderMessages(msgs) {
    ...
    // Note: If msgs exist, welcome element is gone from DOM since we cleared innerHTML.
    // That's fine.
  ...
  function appendMessageUI(msg) {
    ...
    bubble.className = `max-w-[80%] p-3 rounded-2xl text-sm ${isMe
      ? 'bg-gradient-to-br from-indigo-500 to-purple-600 text-white rounded-tr-none'
      : 'bg-white text-slate-700 shadow-md rounded-tl-none border border-slate-100'
      }`;
    bubble.innerText = msg.content;

    const time = document.createElement('span');
    time.className = `text-[10px] block mt-1 ${isMe ? 'text-indigo-100' : 'text-slate-400'}`;
  ...
  function setupSocketListeners() {
    ...
    socket.on('message_sent', (msg) => {
      appendMessageUI(msg); // Confirmation of my own message
      scrollToBottom();
    });
